...
\indent$\cdots$ \\
\indent $W^{n_N}_{t+1} = W^{n_N}_t + \alpha_t\Big(\frac{1}{D}\sum_{n=1}^D \mathbold{\mu}_n^{n_{N-1}}(\mathbold{\mu}_n^{n_N})^T- \frac{1}{M}\sum_{m=1}^M \tilde{\mathbf{h}}_{t+1,m}^{n_{N-1}}(\tilde{\mathbf{h}}_{t+1,m}^N)^T\Big)$\\
