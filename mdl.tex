...
\\
After pretraining is done, use Up-Down algorithm as fine tuning, which is a combination of the training of RBM, and an algorithm called Wake-Sleep algorithm which is for learning of the Sigmoid Belief Network.
...
\end{figure}
Deep Canonically Correlated Autoencoders (DCCAE) [17] is an extension  of DCCA. Compare to DCCA, it could find solutions with higher correlation sum. 
...
\subsubsection{MNIST}
MNIST [30] is a selected set of samples from NIST data set. It has one training data set, one training label set, one testing data set, and one testing label set. The training set has 60,000 samples and the testing set has 10,000 samples. Each sample data is a 28$\times$28 grey image which is a handwritten integer between 0 and 9. It has 10 classes, so the label is between 0 (inclusive) and 9 (inclusive).\\
\\
...
\subsubsection{CIFAR}
The CIFAR-10 data set [15] consists of 60000 32x32 colour images in 10 classes, with 6000 images per class. There are 5 batches of training images and 1 batch of test images, each consists of 10000 images.\\
\\
...
\subsubsection{XRMB}
As stated in its explanation, Wisconsin X-ray Microbeam Database (XRMB) data [27] used in "Deep Canonical Correlation Analysis" [7] is processed by Galen Andrew, Raman Arora, etc.The dimension of the MFCC data is 273 and the dimension of the XRMB data is 112. There are label files of fold 0 that provides the phone labels. The data is stored in double-precision format.\\
\\
