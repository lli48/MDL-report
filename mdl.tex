...
\indent //\textbf{Variational Inference:}\\
\indent \textbf{for} each data sample \textbf{v}$_n,n=1$ to $n_0$ \textbf{do}\\
...
$\theta^{rec}_{t+1} = \theta^{rec}_{t} + \alpha_t
\frac{\partial \text{KL}(Q^{MF}||Q^{rec})}{\partial \theta^{rec}}$\\
Set $\mathbold{\mu}_n = \mathbold{\mu}$. \par
\textbf{end for}\\
\\
\indent //\textbf{Stochastic Approximation:}\\
\indent \textbf{for} each sample $m=1$ to $M$ \textbf{do}\par
Sample $(\tilde{v}_{t+1,m},\tilde{h}_{t+1,m})$ given $(\tilde{v}_{t,m},\tilde{h}_{t,m})$ by running a Gibbs sampler.\par 
\textbf{end for}\\
\\
\indent //Parameter Update:\\
\indent $W^1_{t+1} = W^1_t + \alpha_t\Big(\frac{1}{N}\sum_{n=1}^N \mathbf{v}_n(\mathbold{\mu}_n^1)^T- \frac{1}{M}\sum_{m=1}^M \tilde{\mathbf{v}}_{t+1,m}(\tilde{\mathbf{h}}_{t+1,m}^1)^T\Big)$\\
\indent $W^2_{t+1} = W^2_t + \alpha_t\Big(\frac{1}{N}\sum_{n=1}^N \mathbold{\mu}_n^1(\mathbold{\mu}_n^2)^T- \frac{1}{M}\sum_{m=1}^M \tilde{\mathbf{h}}_{t+1,m}^1(\tilde{\mathbf{h}}_{t+1,m}^2)^T\Big)$\\
\indent $W^3_{t+1} = W^3_t + \alpha_t\Big(\frac{1}{N}\sum_{n=1}^N \mathbold{\mu}_n^2(\mathbold{\mu}_n^3)^T- \frac{1}{M}\sum_{m=1}^M \tilde{\mathbf{h}}_{t+1,m}^2(\tilde{\mathbf{h}}_{t+1,m}^3)^T\Big)$\\
\indent Decrease $\alpha_t$.\\
\noindent \textbf{end for}

\begin{equation}
\mu_j^1 \leftarrow \sigma\Big(\sum_{i=1}^D W_{ij}^1v_i + \sum_{k=1}^{F_2}W_{jk}^2\mu_k^2 \Big),
\end{equation}

\begin{equation}
\nu_j^1 = \sigma\Big( \sum_{i=1}^D 2R_{ij}^1v_i \Big),
\end{equation}
\\
Figure 7.2 shows the algorithm of mean field inference, which is from [5].
\begin{figure*}[!t]
\centering
